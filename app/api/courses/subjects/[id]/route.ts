...
export async function PUT(
    ...
    try {
        ...
        // If section_id is provided, validate and update with it
        if (section_id) {
            await db
                .prepare(`
                    UPDATE subjects 
            ...
            // Update without changing section
            await db
                . prepare(`
                    UPDATE subjects 
        ...
        const logPromise = db
            ...
            .bind(null, 'update', `Updated subject: "${oldName}" to "${name}"`)
            .run();
...
export async function PATCH(
    ...
    try {
        ...
        const logPromise = db
            .prepare('INSERT INTO activity_logs (user_id, action_type, description) VALUES (?, ?, ?)')
            .bind(null, 'update', `Subject "${current.name}" ${statusText}`)
            .run();
...
export async function DELETE(
    ...
    try {
        ...
        const logPromise = db
            . prepare('INSERT INTO activity_logs (user_id, action_type, description) VALUES (?, ?, ?)')
            .bind(null, 'delete', `Deleted subject: ${subjectName}`)
            .run();
